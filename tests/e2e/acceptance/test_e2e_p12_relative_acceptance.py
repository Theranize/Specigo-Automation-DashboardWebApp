"""E2E P12: FD adds relative Vikram Kumar Mishra (Brother) to account 7777777777 → full acceptance flow."""
...
_TEST = "test_e2e_p12_relative_acceptance"
...
def _shot(page, label: str) -> str:
    ...
    path = f"artifacts/p12_{label}.png"
...
def test_e2e_p12_relative_acceptance(page, login_as):
    """P12: FD adds relative (Brother) on 7777777777 → Phlebo → Accession → LT → Doctor."""
    pid = SESSION["e2e_assignments"][_TEST]["patient_id"]

    # ── Phase 1: Front Desk (register patient + add relative) ───────
    login_as("front_desk")
