"""E2E P5: FD adds relative Sunita Kumari Mishra (Wife) to account 8839900148 → full acceptance flow."""
...
_TEST = "test_e2e_p5_relative_acceptance"
...
def _shot(page, label: str) -> str:
    ...
    path = f"artifacts/p5_{label}.png"
...
def test_e2e_p5_relative_acceptance(page, login_as):
    """P5: FD adds relative (Wife) on 8839900148 → Phlebo → Accession → LT → Doctor."""
    pid = SESSION["e2e_assignments"][_TEST]["patient_id"]

    # ── Phase 1: Front Desk (register + add relative Sunita) ─
    login_as("front_desk")
