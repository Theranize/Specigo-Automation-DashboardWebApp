"""E2E P6: 3-cycle rejection — accession rejects Urine, LT rejects Serum, doctor resamples CBC; final full approve."""
...
def _shot(page, label: str) -> str:
    ...
    path = f"artifacts/p6_{label}.png"
...
def test_e2e_p6_rejection(page, login_as):
    """P6: Acc(reject Urine) → LT(reject Serum) → Doctor(resample CBC) → full approve."""
    ...
    # ── Phase 3+4: Accession — reject Urine + reassign ──────────────────────
    print(f"\n[{pid}] --- Phase 3+4: Accession reject Urine + reassign ---")
    login_as("accession")
    with phase_tracker.track(page, pid, "Accession (Reject Urine)", _TEST):
        ...
        _shot(page, "04_reassign_urine")
        if re1_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 4 Reassign Urine: {re1_r['error_message']}")
        assert re1_r["completed"]
        print(f"[{pid}] Phase 3+4 DONE: Accession rejected + reassigned Urine")
    _logout(page)

    # ── Phase 5: Phlebotomist — recollect Urine ──────────────────────────────
    print(f"\n[{pid}] --- Phase 5: Phlebo recollect Urine ---")
    login_as("phlebotomist")
    with phase_tracker.track(page, pid, "Phlebotomist (Recollect Urine)", _TEST):
        rc1_r = execute_recollection_flow(page, rc_e["cycles"][0])
        _shot(page, "05_recollect_urine")
        if rc1_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 5 Recollect Urine: {rc1_r['error_message']}")
        assert rc1_r["completed"]
        print(f"[{pid}] Phase 5 DONE: Urine recollected")
    _logout(page)

    # ── Phase 6: Accession — re-accept Urine ─────────────────────────────────
    print(f"\n[{pid}] --- Phase 6: Accession re-accept Urine ---")
    login_as("accession")
    with phase_tracker.track(page, pid, "Accession (Re-accept Urine)", _TEST):
        ac2_r = execute_accession_flow(page, _with_pid(ac_e["re_accept"][0], pid))
        _shot(page, "06_acc_accept_urine")
        if ac2_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 6 Accession re-accept Urine: {ac2_r['error_message']}")
        assert ac2_r["completed"]
        print(f"[{pid}] Phase 6 DONE: Recollected Urine accepted at accession")
    _logout(page)

    # ── Phase 7: LT — reject Serum ───────────────────────────────────────────
    print(f"\n[{pid}] --- Phase 7: LT reject Serum ---")
    login_as("lab_technician")
    with phase_tracker.track(page, pid, "Lab Technician (Reject Serum)", _TEST):
        ls1_r = execute_labtech_search(page, lt_e)
        _shot(page, "07_lt_reject_serum")
        if ls1_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 7 LT reject Serum: {ls1_r['error_message']}")
        assert ls1_r["completed"]
        print(f"[{pid}] Phase 7 DONE: LT rejected Serum, accepted others")
    _logout(page)

    # ── Phase 8: Accession — reassign Serum ──────────────────────────────────
    print(f"\n[{pid}] --- Phase 8: Accession reassign Serum ---")
    login_as("accession")
    with phase_tracker.track(page, pid, "Accession (Reassign Serum)", _TEST):
        re2_r = execute_reassignment_flow(page, re_e["cycles"][1])
        _shot(page, "08_reassign_serum")
        if re2_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 8 Reassign Serum: {re2_r['error_message']}")
        assert re2_r["completed"]
        print(f"[{pid}] Phase 8 DONE: Serum reassigned to phlebotomist")
    _logout(page)

    # ── Phase 9: Phlebotomist — recollect Serum ──────────────────────────────
    print(f"\n[{pid}] --- Phase 9: Phlebo recollect Serum ---")
    login_as("phlebotomist")
    with phase_tracker.track(page, pid, "Phlebotomist (Recollect Serum)", _TEST):
        rc2_r = execute_recollection_flow(page, rc_e["cycles"][1])
        _shot(page, "09_recollect_serum")
        if rc2_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 9 Recollect Serum: {rc2_r['error_message']}")
        assert rc2_r["completed"]
        print(f"[{pid}] Phase 9 DONE: Serum recollected")
    _logout(page)

    # ── Phase 10: Accession — re-accept Serum ────────────────────────────────
    print(f"\n[{pid}] --- Phase 10: Accession re-accept Serum ---")
    login_as("accession")
    with phase_tracker.track(page, pid, "Accession (Re-accept Serum)", _TEST):
        ...
        _shot(page, "10_acc_accept_serum")
        if ac3_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 10 Accession re-accept Serum: {ac3_r['error_message']}")
        assert ac3_r["completed"]
        print(f"[{pid}] Phase 10 DONE: Recollected Serum accepted at accession")
    _logout(page)

    # ── Phase 11: LT — accept Serum + save all 7 tests ───────────────────────
    print(f"\n[{pid}] --- Phase 11: LT accept Serum + save all 7 tests ---")
    login_as("lab_technician")
    with phase_tracker.track(page, pid, "Lab Technician (Save All)", _TEST):
        ...
        if ls2_r["error_found"]:
            ...
            pytest.fail(f"[{pid}] Phase 11 LT2 accept Serum: {ls2_r['error_message']}")
        ...
        print(f"[{pid}] Phase 11 DONE: LT accepted Serum + saved all 7 tests")
    _logout(page)

    # ── Phase 12: Doctor — resample CBC ──────────────────────────────────────
    print(f"\n[{pid}] --- Phase 12: Doctor resample CBC ---")
    login_as("doctor")
    with phase_tracker.track(page, pid, "Doctor (Resample CBC)", _TEST):
        dr1_r = execute_doctor_flow(page, doc_e)
        _shot(page, "12_doctor_resample")
        if dr1_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 12 Doctor resample CBC: {dr1_r['error_message']}")
        assert dr1_r["completed"]
        print(f"[{pid}] Phase 12 DONE: Doctor resampled CBC")
    _logout(page)

    # ── Phase 13: Accession — reassign Whole Blood/Plasma ────────────────────
    print(f"\n[{pid}] --- Phase 13: Accession reassign WB (doctor resample) ---")
    login_as("accession")
    with phase_tracker.track(page, pid, "Accession (Reassign WB)", _TEST):
        re3_r = execute_reassignment_flow(page, re_e["cycles"][2])
        _shot(page, "13_reassign_wb")
        if re3_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 13 Reassign WB (doctor): {re3_r['error_message']}")
        assert re3_r["completed"]
        print(f"[{pid}] Phase 13 DONE: WB reassigned after doctor resample")
    _logout(page)

    # ── Phase 14: Phlebotomist — recollect Whole Blood/Plasma ────────────────
    print(f"\n[{pid}] --- Phase 14: Phlebo recollect WB ---")
    login_as("phlebotomist")
    with phase_tracker.track(page, pid, "Phlebotomist (Recollect WB)", _TEST):
        rc3_r = execute_recollection_flow(page, rc_e["cycles"][2])
        _shot(page, "14_recollect_wb")
        if rc3_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 14 Recollect WB: {rc3_r['error_message']}")
        assert rc3_r["completed"]
        print(f"[{pid}] Phase 14 DONE: WB recollected")
    _logout(page)

    # ── Phase 15: Accession — re-accept Whole Blood/Plasma ───────────────────
    print(f"\n[{pid}] --- Phase 15: Accession re-accept WB (post-resample) ---")
    login_as("accession")
    with phase_tracker.track(page, pid, "Accession (Re-accept WB)", _TEST):
        ac4_r = execute_accession_flow(page, _with_pid(ac_e["re_accept"][2], pid))
        _shot(page, "15_acc_accept_wb")
        if ac4_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 15 Accession re-accept WB: {ac4_r['error_message']}")
        assert ac4_r["completed"]
        print(f"[{pid}] Phase 15 DONE: Recollected WB accepted at accession")
    _logout(page)

    # ── Phase 16: LT — accept WB + save CBC only ─────────────────────────────
    print(f"\n[{pid}] --- Phase 16: LT accept WB + save CBC ---")
    login_as("lab_technician")
    with phase_tracker.track(page, pid, "Lab Technician (Save CBC)", _TEST):
        lt_rc2 = lt_e["re_cycles"][1]
        ls3_r = execute_labtech_search(page, lt_rc2)
        if ls3_r["error_found"]:
            _shot(page, "16_lt3_search_fail")
            pytest.fail(f"[{pid}] Phase 16 LT3 accept WB: {ls3_r['error_message']}")
        assert ls3_r["completed"]

        lt3_r = execute_labtech_tests(page, lt_rc2)
        _shot(page, "16_lt3_tests")
        if lt3_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 16 LT3 save CBC: {lt3_r['error_message']}")
        assert lt3_r["completed"]
        print(f"[{pid}] Phase 16 DONE: LT accepted WB + saved CBC")
    _logout(page)

    # ── Phase 17: Doctor — approve CBC ───────────────────────────────────────
    print(f"\n[{pid}] --- Phase 17: Doctor approve CBC ---")
    login_as("doctor")
    with phase_tracker.track(page, pid, "Doctor (Approve CBC)", _TEST):
        dr2_r = execute_doctor_flow(page, _with_pid(doc_e["re_approval"], pid))
        _shot(page, "17_doctor_approve")
        if dr2_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 17 Doctor approve CBC: {dr2_r['error_message']}")
        assert dr2_r["completed"]
        for t in dr2_r["test_results"]:
            assert t["error"] is None, f"Doctor approve error: {t['test_name']} - {t['error']}"
        print(f"[{pid}] Phase 17 DONE: Doctor approved CBC")
    _logout(page)
