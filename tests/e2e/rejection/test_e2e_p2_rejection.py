"""E2E P2: 3-cycle rejection — accession rejects Serum, LT rejects 24h urine, doctor resamples LFT; final partial_approve."""
...
def _shot(page, label: str) -> str:
    ...
    path = f"artifacts/p2_{label}.png"
...
def test_e2e_p2_rejection(page, login_as):
    """P2: Acc(reject Serum) → LT(reject 24h urine) → Doctor(resample LFT) → partial_approve."""
    ...
    # ── Phase 17: Doctor — partial_approve LFT ───────────────────────────────
    print(f"\n[{pid}] --- Phase 17: Doctor partial_approve LFT ---")
    login_as("doctor")
    with phase_tracker.track(page, pid, "Doctor (Partial Approve LFT)", _TEST):
        dr2_r = execute_doctor_flow(page, _with_pid(doc_e["re_approval"], pid))
        _shot(page, "17_doctor_partial_approve")
        if dr2_r["error_found"]:
            pytest.fail(f"[{pid}] Phase 17 Doctor partial_approve LFT: {dr2_r['error_message']}")
        assert dr2_r["completed"]
        for t in dr2_r["test_results"]:
            assert t["error"] is None, f"Doctor partial_approve error: {t['test_name']} - {t['error']}"
        print(f"[{pid}] Phase 17 DONE: Doctor partial_approved LFT")
    _logout(page)
